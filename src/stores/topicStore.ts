...
import { recordWledMessage } from "../utils/ecosystems/wled";
import {
  isSparkplugTopic,
  parseSparkplugTopic,
  sparkplugDeviceKey,
...
interface TopicStoreState {
  ...
  /** Whether to show WLED light-controller indicator rings in the graph. */
...
export const useTopicStore = create<TopicStoreState>((set, get) => {
  ...
  showWledIndicators: saved.showWledIndicators ?? cfg.showWledIndicators ?? true,
  ...
  handleMessage: (topic: string, payload: string, qos: 0 | 1 | 2, retain = false, properties?: MqttV5Properties, imageBlobUrl?: string, rawPayload?: ArrayBuffer) => {
    ...
    const structuralHit =
      ...
      recordWledMessage(_entityRegistry, topic, node.id, payload);
    if (structuralHit) {
      mergeNodeTag(node, {
  ...
  resetSettings: () => {
    ...
    set({
      ...
      showWledIndicators: cfg.showWledIndicators ?? true,
