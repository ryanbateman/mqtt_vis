...
- **Every non-empty payload is submitted**; a per-node 500ms THROTTLE (leading + trailing edge, latest payload wins — NOT a restarting debounce, which would starve fast publishers) plus the identical-payload skip (length + FNV-1a fingerprint in `payloadAnalyzerService`) keep worker load bounded. Payloads are sliced to 64 KB (`prepareAnalysisPayload` in `src/utils/payloadAnalysis.ts`) before posting.
...
- **UI**: emerald ring (family-wide red/dashed when offline via `GraphRenderer.setSparkplugOfflineNodes`), DetailPanel "View Device" button, Insights Drawer "Device" tab (`SparkplugDevicePanel.tsx`: status, last birth/data, seq + gaps, live metric table with per-metric sparklines).
- **Metric sparklines**: history (`appendMetricHistory`, 60 samples/metric, numeric + boolean only — booleans stored 0/1 and step-rendered) is recorded ONLY while a device panel is open (`startSparkplugHistory`/`stopSparkplugHistory`, one device at a time, module-level buffer in `topicStore`). Sparklines start empty on open by design. Sample timestamps prefer metric ts → payload ts → now.
...
- **Performance with many topics**: SVG can handle hundreds of nodes but may struggle above ~1000 (measured: d3 tick ~87 ms at ~9000 nodes). The tree is hard-capped at `TOPIC_NODE_CAP` (2000, `topicStore.ts`): beyond it, messages for new topics are dropped (existing topics keep updating), a StatusBar banner tells the user to narrow the filter, and pruning frees capacity again. The live node count is tracked in module var `_treeNodeCount` (incremented on create, decremented on prune) because `totalTopics` is cumulative. The proper fix beyond the cap is Canvas rendering (issue #19).
